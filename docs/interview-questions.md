...
**A:** The content is immutable — the Quran text and tafsir don't change between pipeline runs. Shipping 114 static JSON files under `web/public/data/` means the "API" is just a static file server (GitHub Pages), which is free and infinitely scalable. A backend API would add latency, hosting cost, and a failure surface for data that never changes dynamically. The only dynamic data (favorites, future bookmarks) is per-user and either lives in localStorage now or will live in Supabase later. Static files for content + Supabase for user data is the right split.

### Q3. Walk me through the full data pipeline from source to browser.

**A:** Two upstream sources feed the Python `pipeline/`: Quran text (Uthmani XML from Tanzil.net, fetched + disk-cached by `quran/fetcher.py`, parsed by `quran/parser.py`) and tafsir lessons (scraped from nabulsi.com by `tafsir/scraper.py`, with `tafsir/lesson_parser.py` using Arabic regex to map lesson titles like "الآيات 1-5" → ayah ranges, and `content_extractor.py` cleaning the HTML body). `merge/builder.py` joins Quran + tafsir + optional `media/mapper.py` links into one JSON per surah. Output (`output/1.json`…`114.json` + `_index.json` + `_report.json`) is copied into `web/public/data/` via the `copy-data` script. At runtime the SPA `fetch()`es these files on demand.

### Q4. Why copy `pipeline/output/` into `web/public/data/` instead of pointing the app at a shared path?

**A:** `web/public/` is the only directory Vite serves as static assets at the deployed root. The pipeline lives outside `web/` (it's a separate Python project with its own `pyproject.toml`). Bundling generated data into the app's public dir makes the `web/` folder self-contained — `pnpm build` produces a deployable artifact with its data baked in, decoupled from the pipeline. The `copy-data` npm script (`cp ../pipeline/output/*.json public/data/`) is the explicit bridge. An alternative would be a symlink, but symlinks break on Windows and some CI/deploy environments.
...
### Q71. Does fetching `/data/*.json` from the same origin cause CORS issues? Why or why not?

**A:** No. The JSON files are served from the same origin as the app (both under the deployed root on GitHub Pages), so the browser's same-origin policy permits the requests freely — no CORS headers needed. CORS only bites on *cross-origin* requests (different scheme/host/port). One caveat: in dev (`vite` on port 5173) vs prod (GitHub Pages), the base path matters — Vite's `base: '/tafseer-nabulsi/'` config ensures `/data/...` resolves correctly under the project subpath in production. Getting `base` wrong is the classic GitHub-Pages deployment bug.
...
### Q101. The data JSON was recently minified, creating a 54,000-line diff that obscures real content changes. How would you keep data PRs reviewable?

**A:** Store the JSON pretty-printed in the repo (readable diffs, reviewable PRs) and minify as a build step. Options: (1) a `prebuild` script that minifies `public/data/*.json` into `dist` (Vite already serves from `dist` in production). (2) Configure the pipeline's `merge/builder.py` to emit pretty JSON, and let Vite's asset pipeline handle compression. (3) Keep two representations — pretty in repo, minified in `gh-pages` — generated at deploy time. The principle: optimize for human review in source control, machine transfer in production. The current single-line format optimizes the latter at the cost of the former.
